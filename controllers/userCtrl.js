...
register: async (req,res)=>{
   try{
      ...
      if(user) return res.status(400).json({msg:"This Email is Alrady Exist"})

      if(password.length < 6)
      return res.status(400).json({msg:"Password at least 6 character longer"})
...
login:async (req,res)=>{
   try{
      ...
      // if login success , create access token and reefresh token
